...
module button_hold_detect #(
    ...
    localparam int CountMax = HOLD_CYCLES - 1; // Count from 0 to HOLD_CYCLES-1
    ...
    always_comb begin
        if (button) begin
            ...
            if (held)
                ...
                count_enable = 1; // button hasnt already been held, so start counting!
        end else begin
    ...
    always_comb begin
        if (32'(count) == CountMax)
            held = 1; // Button has been held for the required duration
        else
            held = 0; // Button has not been held long enough
    end
endmodule
